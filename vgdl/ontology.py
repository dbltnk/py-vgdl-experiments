...
class CursorAvatar(MovingAvatar, SpriteProducer):
    """ Creates a sprite on the cell where it currently is """
...
class SelectedPiece(MovingAvatar):
    """ Can move one space and then becomes a cursor again. """
    ...
    def update(self, game):
        MovingAvatar.update(self, game)
        if self.rect.left <> self.x or self.rect.top <> self.y:
            killSprite(self, None, game)
            game._createSprite([self.stype], (self.rect.left, self.rect.top))
